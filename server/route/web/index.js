module.exports = app => {
  ...
  router
    .get('/news/init', async (req, res) => {
      const parent = await Category.findOne({name: '新闻分类'})
      const cats = await Category.find().where({parent}).lean()
      const newsTitls = ["新英雄爆料丨蒙犽：我的子弹会拐弯，我的想法不一般！", "玄雍资料片1月上线，嬴政形象升级抢先看", "峡谷交响乐幕后故事揭秘（交响音乐会倒计时2天）", "皮肤爆料 | S18赛季皮肤提前曝光，竟然是ta！", "星元上新丨英雄齐换装，圣诞头饰、武器抢先看", "12月13日体验服停机更新公告", "12月10日全服不停机更新公告", "部分微信区服闪退及对局结算异常说明公告", "12月11日全服不停机更新公告", "【已修复】圣诞礼盒活动异常问题处理公告", "冰雪聚峡谷 轻松得好礼", "【看KPL秋决，赢豪华大礼】活动公告", "【奇遇咕嗒的寻宝之旅】活动公告", "【已修复】圣诞礼盒活动异常修复公告", "圣诞礼盒活动公告", "2019年KPL秋季赛总决赛明日开启 线上观赛赢海量福利", "荣耀巅峰，该我上场！六福珠宝设计制作全新KPL冠军戒指揭秘", "C4D视觉打造的王者校园，慎点！你可能被炫到~", "高校海选赛报名最后一周，为校而战就等你来", "模拟战冬季冠军杯分组情况及正赛赛程公布"]
    ...
    .get('/news/list', async (req, res) => {
      ...
      const cats = await Category.aggregate([
        {$match: {parent: parent._id}}, // 查询父类id
        { // 关联 articles 查询 categories 中与当前数据 id 相同的数据，并命名为 newsList
          $lookup: {
            from: 'articles',
            localField: '_id',
